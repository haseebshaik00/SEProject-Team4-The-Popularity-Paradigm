...
to_email_path = '../aggregated_data/emails/'
to_commit_path = '../aggregated_data/commits/'
...
for project in tqdm(projects):
	...
	# dividing emails
	with open(to_email_path + project_id + '.csv', 'w') as f:
		pass
	...
	for line in lines[1:]:
		...
		with open(to_email_path + project_id + '.csv', 'a') as f:
			f.write(','.join(things) + '\n')
		
	# dividing commits
	with open(to_commit_path + project_id + '.csv', 'w') as f:
		pass
	...
	for line in lines[1:]:
		...
		# line = [str(l) for l in line]
		with open(to_commit_path + project_id + '.csv', 'a') as f:
			f.write(','.join(things)+ '\n')
